...
func TestBaselineDecidesWhetherAParameterCanBeBlamed(t *testing.T) {
	for _, test := range []struct {
		...
		{"the operation works as documented", "200", 200, true},
		{"documented as an error, and errors", "404", 404, true},
		{"documented as 200, answers 404", "200", 404, false},
		{"documented as 200, answers 500", "200", 500, false},
		{"documented as 404, answers 200", "404", 200, false},
		// A description with no usable status falls back to "did it not error",
		// which is the most that can be said without one.
		{"no documented status, answers 200", "", 200, true},
		{"no documented status, answers 500", "", 500, false},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if got != test.want {
				t.Errorf("baselineWorks = %v, want %v (documented %q, answered %d)",
					got, test.want, test.documented, test.answers)
			}
		})
	}
}
...
func TestAnUnreachableServerIsNotABaseline(t *testing.T) {
	...
	if baselineWorks(context.Background(), runner.New("http://127.0.0.1:1"), transaction) {
